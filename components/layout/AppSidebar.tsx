...
import type { MenuItem, SidebarRole } from "@/types/ui/sidebar";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
const AppSidebar = ({
  ...
  const normalizedRole = role === "employee" ? "worker" : role;
  const menus = sidebarConfig[normalizedRole] || sidebarConfig.admin;
                    ...
                    <CollapsibleContent>
                      <SidebarMenuSub className="ml-4 mt-1 border-l border-border/60 pl-2 gap-1">
                        {menu.submenu.map((sub) => {
                          ...
                          return (
                            <SidebarMenuSubItem key={`${menu.title}:${sub.title}:${sub.path}`}>
